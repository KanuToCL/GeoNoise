# GeoNoise - Developer Onboarding Guide
...
### Entity: [PointSource](packages/core/src/schema/index.ts#L89)
...
### Entity: [PointReceiver](packages/core/src/schema/index.ts#L118)

- **Schema**: `PointReceiverSchema`
- **Fields**:
  - `id: string` - Unique identifier
  - `type: 'point'` - Discriminator
...
### Entity: [RectangularPanel](packages/core/src/schema/index.ts#L143)

- **Schema**: `RectangularPanelSchema`
- **Fields**:
  - `id: string` - Unique identifier
...
### Entity: [Barrier](packages/core/src/schema/index.ts#L195)
...
### Entity: [Building](packages/core/src/schema/index.ts#L182)
...
### Entity: [Scene](packages/core/src/schema/index.ts#L284)

- **Schema**: `SceneSchemaV1`
- **Fields**:
...
### Entity: [PropagationConfig](packages/core/src/schema/index.ts#L248)

- **Schema**: `PropagationConfigSchema`
- **Fields**:
...
### Entity: [Spectrum9](packages/shared/src/utils/index.ts#L214)
...
### Spreading Loss
```
Spherical (point source):  A_div = 20·log₁₀(r) + 11 dB
Cylindrical (line source): A_div = 10·log₁₀(r) + 8 dB
```
...
where α = frequency-dependent coefficient from ISO 9613-1
```

### Ground Effect (ISO 9613-2 Eq. 10)
```
h_m = (h_s + h_r) / 2
...
Blocked:   A_total = A_div + A_atm + max(A_bar, A_gr)
```
...
where L_w = source sound power level (dB re 1 pW)
```

### A-Weighted Overall (LAeq)
```
LAeq = 10·log₁₀[Σᵢ 10^((Lᵢ + Aᵢ)/10)]

where Lᵢ = band level, Aᵢ = A-weighting for band i
```
...
├── packages/
│   ├── core/                   # Schemas & types
│   ├── engine/                 # Propagation physics
│   ├── engine-backends/        # CPU/GPU backends
│   ├── engine-webgpu/          # WebGPU backend
...
*Generated by Devmate on 2026-01-06*
